...
- `data/events.json` + `data/schools.json` — events hub data (ship empty; real entries added via the curator tool). Event fields: `id`, `title`, `type` (`social`/`warsztaty`/`festiwal`), `styles` (dance slugs), `city`, `date_start`, optional `date_end`/`time`/`venue`/`price`/`image`, `url`, `featured`, `organizer`, `source`.
- `tools/add-event.js` — dependency-free Node curator CLI that imports an event from a public URL's Open Graph tags (or manual entry), validates it, and inserts it chronologically into `data/events.json`. Its pure functions are unit-tested — run `node --test tools/add-event.test.js` (the directory form `node --test tools/` is broken on Node v26) before claiming tool changes complete. Publishing an event = run the tool, then git commit/push.
